...
**已从 src/ 迁移到可交付的 skill 包**：

```
skill/revenue-recognition/scripts/lib/
```

`src/` 现在只承载本地工具，不再包含汇总业务逻辑：

- `src/cli/run_skill_eval_batch.py` —— eval-harness CLI 入口。
- `src/evals/` —— WorkBuddy / CodeBuddy eval harness 核心。
- `src/integrations/codebuddy_cli/` —— CodeBuddy CLI 调用集成。
...
包内的业务代码。`cli` 与 `integrations` 是 PEP 420 命名空间包，可同时解析 src（eval-harness）
