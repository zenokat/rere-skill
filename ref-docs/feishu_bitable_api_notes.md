...
- ???????????? `create_field`?????? `BitableApiClient._request(...)` ?????
- `type=2` ????? Number ???
...
```json
{
  ...
  "property": {
    "formatter": "0.0",
    "formula_expression": "bitable::$table[tblxxxx].$field[fldA]+bitable::$table[tblxxxx].$field[fldB]",
    "type": {
      "data_type": 2,
      "ui_property": {
        "formatter": "0.0"
...
### E. ??????? `optional` ??
- ???????????????????????????
- ????? `validate` ?????? `optional=false`?`preview` ???`SUM field 'xxx' is empty.`
- ?????????? SUM ???????? `optional=true`????? 0 ?????

### F. read_only workbook ? stale dimension
- ?????? `????` ??????
  `openpyxl` ? `read_only=True` ????????? worksheet dimension??? `A1:BH8`?
- ???? streaming preview ????????? `validate` ??? `preview` ???????
- ?????
  - ? read_only worksheet ????? `max_row == 1` ?? `reset_dimensions()`?
  - ? streaming ?????? `reset_dimensions()`????? `calculate_dimension(force=True)`?
