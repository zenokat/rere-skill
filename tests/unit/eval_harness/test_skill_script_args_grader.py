...
def test_passes_when_uv_run_script_flag_invocation_has_target_arg(tmp_path: Path) -> None:
    """Score 1 when ``uv run --script`` carries the expected argument."""

    session = _write_session(tmp_path, [
        _bash_event(
            'uv run --script ".workbuddy/skills/revenue-recognition/scripts/run_recog_rollup.py" '
            '--validate --recog_id eleme_delivery_revenue --source_file "input/饿了么收入"'
        ),
    ])
    result = grade_skill_script_args(
        session_jsonl=session, script_name="run_recog_rollup", arg_pattern="--validate",
